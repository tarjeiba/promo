...
#+begin_src python
def avstand(fart, tid):
    """Tar inn fart i kilometer i timen og tid i timer og returnerer avstanden i kilometer. 

    Regnet ut ved å bruke at avstand = fart * tid, altså antas det null akselerasjon.
    """
    return fart * tid
#+end_src
...
#+begin_export html
<iframe width="560" height="315" src="https://www.youtube.com/embed/exQLowWfq0w" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
#+end_export

Du finner den resulterende fila [[py:vokalteller.py][her (vokalteller.py)]].

* Videre med terminprosjekt og klassetursinformasjon
  :PROPERTIES:
  :custom_id: klasseturinfo
  :END:
...
*** Sted
[[https://www.nosenyoga.no/][Nøsen yoga]] i Valdres.
...
En e-post til [[mailto:tabaa004@osloskolen.no][tabaa004@osloskolen.no]] i løpet av tirsdag 3. desember dersom dere /ikke/ har mulighet til å være med.
...
- Last ned [[py:arduino_serial.py][arduino\under{}serial.py]], og legg denne i arbeidsmappa di.
...
tider, ekkotider = collect_data(runtime=5000, delay=2000, comport="COM4")
...
tall = [1, 2, 5, -5, 18, 17.3, math.pi]
dobbelttall = []

for t in tall:
    dobbelttall.append( f(t) )
...
- Gjør en fullstendig gjennomkjøring for hvert målepunkt, men du kan bruke kort måletid. For eksempel vil det holde med ~collect_data(runtime=1500)~ for kun å ta målinger i halvannet sekund
- Tegn opphvordan sensorens målte verdier samsvarer med virkelige verdier (dette kan du gjøre for hånd)
