...
* uke 19
** Mappeinnlevering
Som sagt under gårsdagens time, skal «mappa» deres leveres til førsteutkastsvurdering onsdag om ei uke. Lagre alle filene i en ~zip~-fil og send denne til meg på e-post [[mailto:tabaa004@osloskolen.no][tabaa004@osloskolen.no]]. I mappa legger dere minst følgende:

- twitterbotbesvarelsen deres
- sprettballsimuleringen deres
- gasspartikkelsimulering (eller en utvidet utgave som viser brownsk bevegelse)
- difflikningløsningen fra i går ([[https://www.youtube.com/watch?v=p_di4Zn4wz4][3Blue1Brown på Youtube]]) eller [[*Utregninger av pi][π-utregninger]].

Jeg kommer til å gi tilbakemelding i løpet av eksamensuka deres, så dere har én uke fra 22. til 29. mai med å gjøre eventuelle utbedringer. Karakteren settes i løpet av timen 29. mai.

Tilbakemeldingen kommer til å være i form av en videogjennomgang av koden deres, hvor dere kommer til å få oppfølgingsoppgaver. Jeg kommer til å la hovedtyngden av av videoen knytte seg til ett program, fortell meg hvilket program dette skal være, om du har noen formening.

** Vurderingskriterier
Ved en eventuell eksamen i faget, kommer dere til å bli vurdert etter disse [[pdf:pmx_v19_vurderingskriterier.pdf][vurderingskritereiene]].

** Utregninger av pi
Se på videoen under og skriv av koden mens den går (Youtube har pauseknapp). Til den tredje oppgaven trenger du papir og noe å tegne med, fortrinnsvis en relativt tjukk tusj.

#+BEGIN_EXPORT html
<iframe width="560" height="315" src="https://www.youtube.com/embed/BWlR9YDb2I0" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
#+END_EXPORT

«Muntligoppgaver» dere ville fått til oppgaver à la denne:
- Utvid det første programmet til først å gi seg når vi har kommet så nærme som vi ønsker. For eksempel, gå først ut av ~while~-løkka når endringen gjennom to iterasjoner er tilstrekkelig liten.
- Utvid det andre programmet til å først samle de tilfeldige ~x~- og ~y~-verdiene i hver sine lister, som du så plotter til slutt, etter hele ~while~-løkka. Forklar hvorfor det gjør at programmet da går mye raskere.
- Dersom vi kun «cropper» bildet, uten å endre fargesammensetningen, hvordan påvirker vaget av «er svart»-verdi (her ~0.5~) hva vi får som utregnet π? (Her må du kjøre programmet flere ganger, eller lage en ny løkke, hvor du endrer grensen gradvis og rapporterer utregnet verdi.)

* uke 16
** En statusoppdatering
Grunnet mais mange helligdager, og deres instidighet vedrørende heldagsprøver, har vi kun tre økter igjen før karakterene deres skal settes 29. mai. Dette er to økter færre enn  jeg hadde planlagt for, en brøler jeg tar fullt og helt på mine skuldre.

** 8. mai
Her jobber vi med å finne nullpunkter til likninger. Dette er nyttig innenfor mange deler av numerikken (matematikk hvor vi gjør utregninger heller enn å løse oppgavene algebraisk), og vi skal se spesielt på to metoder. Halveringsmetoden og Newton-Raphson-metoden.

Dette er en fortsettelse på ballsimulatoren vi lagde etter vinterferien, og jeg har startet på en gjennomgang av ulike modeller for løsning av et skrått kast [[file:promo.org::*Prosjekt:%20Ballkast][her (Prosjekt: Ballkast)]].

** 15. mai
Differensiallikninger, likninger hvor den derivert dukker opp som en ukjent, løses via en form for numerisk integrasjon.

Denne økta blir ulik avhengig av om dere har matematikk R2 eller ei.

Vi kommer uansett til å skulle løse en modell av à la harer-og-rever-i-et-økosystem, som dere hadde om i naturfag. Her blir det også en videreføring av partikkelmodellen med kolisjoner vi har jobbet med de siste ukene. (Partiklenes farge kan representere hva slags art, kollisjoner mellom to gjenstander med lik farge gjør én ting, ulik farge noe annet.)

** 29. mai
Avslutning, «mappegjennomgang» og karaktersetting.

** Forventninger
*Til meg*
Før 8. mai, og rett etter 8. og 15. mai spiller jeg inn en tilbakemeldingsvideo til hver av dere om hva dere skal gjøre den neste uka.

*Til dere*
Dere setter av et par timer til å jobbe med tilbakemeldingene mellom hver økt. 


** Konsekvens
Konsekvensen av alt dette er at vi ikke får hatt et forsøk hvor dere får sammenlignet modelleringsdata med forsøksdata, så dette er noe vi kan gjøre i perioden mellom karaktersetting og eksamen.

God ferie, programører!

* uke 14
** Innleveringsoppgave
Du kan ta utagangspunk i pythonfila under.
- [[py:fysikkmotor_partikler.py]]

Kontroller at fila fungerer i ditt oppsett.

** a) Legg til masse
Partiklene våre skal ha masse. Legg dette til som en attributt til ~__init__~-metoden til ~Partikkel~-klassen. Gi denne samme verdi som ~self.strlse~.

** b) Endre kollisjoner
I utagangspunktet har vi i ~kollider~-funksjonen vår følgende ~if~-test:
#+BEGIN_SRC python
...
For å ta hensyn til at partiklene nå har ulik masse, kan vi endre kollisjonskoden til følgende:

#+BEGIN_SRC python
if dist < p1.strlse + p2.strlse:
    vinkel = math.atan2(dy, dx) + 0.5 * math.pi
    total_masse = p1.masse + p2.masse
    p1.vinkel, p1.fart = vektorsum(
        (p1.vinkel, p1.fart * (p1.masse - p2.masse) / total_masse),
        (vinkel, 2 * p2.fart * p2.masse / total_masse))
    p2.vinkel, p2.fart = vektorsum(
        (p2.vinkel, p2.fart * (p2.masse - p1.masse) / total_masse),
        (vinkel + math.pi, 2 * p1.fart * p1.masse / total_masse))
#+END_SRC

** c) En stor, mange små
Lag et oppsett hvor du genererer hundre små partikler og én stor.

** d) Tegn kun den store
Gjør så du kun tegner den store partikelen.

* uke 12
** Løsningsforslag til test
Jeg har laget et løsningsforslag til forrige ukes test ... forsøksvis som en screencast.

#+BEGIN_EXPORT html
<iframe width="560" height="315" src="https://www.youtube.com/embed/hJ523bbaohw" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
#+END_EXPORT
Du finner løsningsforslaget som [[py:uke11_lf.py][python-fil her]].
...
- ~for~-løkker
- *Hente eksperimentelle data fra fil* (Gå gjennom avsnittet [[file:promo.org::#tekstfiler][Tekstfiler]], dere har også tilgang til dette på onsdag.)
